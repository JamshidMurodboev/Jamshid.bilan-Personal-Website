...
export default function TelegramContactButton({ children, className, platform = 'telegram', scholarshipContext, universityContext, serviceContext }: Props) {
  ...
  const labelClass = 'block text-xs font-bold uppercase tracking-widest text-muted-e mb-1.5';
  const errorClass = 'text-red-600 dark:text-red-400 text-xs mt-1';
        ...
        >
          <div className="bg-card border border-card rounded-3xl w-full max-w-md shadow-2xl shadow-black/20 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-5 border-b border-line">
              <h2 className="font-display text-lg text-heading">{platformTitle}</h2>
              <button
                type="button"
                onClick={handleClose}
                aria-label="Close"
                className="w-9 h-9 flex items-center justify-center rounded-full border border-line text-muted-e hover:border-accent hover:text-accent transition-colors text-xl leading-none"
              >
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5 max-h-[75vh] overflow-y-auto">
                ...
                <div>
                  <label className={labelClass}>{t('name')} <span className="text-accent">*</span></label>
                ...
                <div>
                  <label className={labelClass}>{t('dob')} <span className="text-accent">*</span></label>
                ...
                <div>
                  <label className={labelClass}>{t('cert')} <span className="text-accent">*</span></label>
                  <label className="flex items-center gap-2 mb-3 cursor-pointer select-none">
                    <input type="checkbox" checked={noCert} onChange={e => { setNoCert(e.target.checked); if (e.target.checked) setErrors(err => ({ ...err, certName: '', certScore: '' })); }} className="w-4 h-4 rounded accent-[var(--accent)]" />
                    <span className="text-sm text-body">{t('noCert')}</span>
                  </label>
                  {!noCert && (
