...
function buildMessage(name: string, dob: string, target: string, other: string) {
  const t = target === 'other' ? other : target;
  return encodeURIComponent(
    `Assalomu Alaykum, Jamshid aka!\n\nIsm: ${name}\nTug'ilgan sana: ${dob}\nAriza: ${t}\n\nMaslahatchingiz bilan bog'lanmoqchiman.`
  );
}
...
export default function HomeContactSection() {
  ...
  function openTelegram() {
    if (!validate()) return;
    const msg = buildMessage(name, dob, target, other);
    window.open(`https://t.me/jamshid_bilan?text=${msg}`, '_blank');
  }

  function openWhatsApp() {
    if (!validate()) return;
    const msg = buildMessage(name, dob, target, other);
    window.open(`https://wa.me/905052250893?text=${msg}`, '_blank');
  }
    ...
    <section className="py-16 px-4 bg-white dark:bg-gray-900" id="contact">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Grant yoki Universitet *</label>
            <select
              value={target}
          ...
          {target === 'other' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Boshqa variant</label>
              <input
                type="text"
                value={other}
