...
const emptyForm = { title_uz: '', title_ru: '', title_en: '', body_uz: '', body_ru: '', body_en: '', cover_url: '', published: false, published_at: '' }
...
export default function NewsPage() {
  ...
  async function load() {
    setLoading(true)
    const { data, error } = await createClient().from('news_posts').select('*').order('created_at', { ascending: false })
  ...
  function openEdit(item: NewsPost) {
    setEditId(item.id)
    setForm({ title_uz: item.title_uz, title_ru: item.title_ru ?? '', title_en: item.title_en ?? '', body_uz: item.body_uz, body_ru: item.body_ru ?? '', body_en: item.body_en ?? '', cover_url: item.cover_url ?? '', published: item.published, published_at: item.published_at ? item.published_at.slice(0, 16) : '' })
    setError(null); setShowModal(true)
  }

  async function handleSave(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError(null)
    const payload = { title_uz: form.title_uz, title_ru: form.title_ru || null, title_en: form.title_en || null, body_uz: form.body_uz, body_ru: form.body_ru || null, body_en: form.body_en || null, cover_url: form.cover_url || null, published: form.published, published_at: form.published_at || null }
    const supabase = createClient()
    const res = editId ? await supabase.from('news_posts').update(payload).eq('id', editId) : await supabase.from('news_posts').insert(payload)
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{item.published_at ? new Date(item.published_at).toLocaleDateString('uz-UZ') : '—'}</td>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) setShowModal(false) }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-800">{editId ? 'Yangilikni tahrirlash' : 'Yangi yangilik'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-3">
              ...
              <div className="flex items-center gap-2"><input id="news-pub" type="checkbox" checked={form.published} onChange={e => setForm({...form, published: e.target.checked})} className="w-4 h-4 accent-teal-700" /><label htmlFor="news-pub" className="text-sm font-medium text-gray-700">Chop etilgan</label></div>
