            ...
            <form onSubmit={handleSave} className="p-6 space-y-3">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-semibold text-gray-700 dark:text-gray-300">Savol *</label>
                  <TranslateFieldButton value={form.question_uz} onResult={(ru, en) => setForm(f => ({ ...f, question_ru: ru, question_en: en }))} />
                </div>
                <div className="flex items-center gap-1"><span className="text-xs text-gray-400 w-6">🇺🇿</span><input required value={form.question_uz} onChange={e => setForm({...form, question_uz: e.target.value})} className={`${inp} flex-1`} /></div>
                <div className="flex items-center gap-1"><span className="text-xs text-gray-400 w-6">🇷🇺</span><input value={form.question_ru} onChange={e => setForm({...form, question_ru: e.target.value})} className={`${inp} flex-1`} /></div>
                <div className="flex items-center gap-1"><span className="text-xs text-gray-400 w-6">🇬🇧</span><input value={form.question_en} onChange={e => setForm({...form, question_en: e.target.value})} className={`${inp} flex-1`} /></div>
              </div>

              {/* Javob */}
              <div className="space-y-1">
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-semibold text-gray-700 dark:text-gray-300">Javob *</label>
                  <TranslateFieldButton value={form.answer_uz} onResult={(ru, en) => setForm(f => ({ ...f, answer_ru: ru, answer_en: en }))} />
                </div>
                <div className="flex items-start gap-1"><span className="text-xs text-gray-400 w-6 pt-2">🇺🇿</span><textarea required rows={3} value={form.answer_uz} onChange={e => setForm({...form, answer_uz: e.target.value})} className={`${inp} flex-1`} /></div>
                <div className="flex items-start gap-1"><span className="text-xs text-gray-400 w-6 pt-2">🇷🇺</span><textarea rows={3} value={form.answer_ru} onChange={e => setForm({...form, answer_ru: e.target.value})} className={`${inp} flex-1`} /></div>
                <div className="flex items-start gap-1"><span className="text-xs text-gray-400 w-6 pt-2">🇬🇧</span><textarea rows={3} value={form.answer_en} onChange={e => setForm({...form, answer_en: e.target.value})} className={`${inp} flex-1`} /></div>
              </div>
