  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Ko'p so'raladigan savollar</h1>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-gray-600 font-medium">Savol</th>
                <th className="text-left px-4 py-3 text-gray-600 font-medium">Javob</th>
                <th className="px-4 py-3"></th>
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 font-medium text-gray-800 max-w-xs">{item.question_uz}</td>
                  <td className="px-4 py-3 text-gray-600 max-w-sm truncate">{item.answer_uz}</td>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) setShowModal(false) }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-800">{editId ? 'Savolni tahrirlash' : "Yangi savol"}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-3">
              ...
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Savol (UZ) *</label><input required value={form.question_uz} onChange={e => setForm({...form, question_uz: e.target.value})} className={inp} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Savol (RU)</label><input value={form.question_ru} onChange={e => setForm({...form, question_ru: e.target.value})} className={inp} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Savol (EN)</label><input value={form.question_en} onChange={e => setForm({...form, question_en: e.target.value})} className={inp} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Javob (UZ) *</label><textarea required rows={3} value={form.answer_uz} onChange={e => setForm({...form, answer_uz: e.target.value})} className={inp} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Javob (RU)</label><textarea rows={2} value={form.answer_ru} onChange={e => setForm({...form, answer_ru: e.target.value})} className={inp} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Javob (EN)</label><textarea rows={2} value={form.answer_en} onChange={e => setForm({...form, answer_en: e.target.value})} className={inp} /></div>
