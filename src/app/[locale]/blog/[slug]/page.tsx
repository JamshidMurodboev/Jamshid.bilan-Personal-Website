  ...
  return (
    <div className="min-h-screen bg-[#faf7f2] dark:bg-gray-950 py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href={`/${locale}/blog`} className="text-sm text-teal-700 dark:text-teal-400 hover:underline">&larr; {t('back')}</Link>
        {post.cover_url && (
        ...
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-6">
            {post.tags.map((tag) => (
              <span key={tag} className="bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 text-xs px-2 py-0.5 rounded-full">{tag}</span>
            ))}
          </div>
        )}
